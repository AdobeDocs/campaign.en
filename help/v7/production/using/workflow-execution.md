...
1. Check the  technical workflow status. This workflow manages jobs or workflows inside a campaign. If it fails, this will result in workflows to not start / stop. Restart it to resume the running of campaign workflows.

    For more on technical workflows monitoring, refer to .
...
1. Check the **). This process is responsible for running all workflows.
...
1. Check if the **number of campaign processes running** on the instance is more than the threshold. There is a limit defined by the  option on how many campaign processes can run on the instance in parallel. When this limit is reached, the workflow stays in the "Start as soon as possible" state as long as the number of workflows running is above the limit.
    ...
    To check the number of workflows running of your instance, we recommend using the predefined views, accessible by default in the **.
...
For more on how to monitor you workflows, refer to .
...
1. Check the **).
   ...
   ```
    ...
    For more on how to monitor modules, refer to .
...
1. Check the workflow journal. For more on this, refer to the  sections.
1. Monitor technical workflows. For more on this refer to the .
