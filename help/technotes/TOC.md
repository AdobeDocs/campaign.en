---
audience: end-user
user-guide-title: Campaign Technotes
type: Event
title: Adobe Campaign Technotes
description: Campaign Technotes
breadcrumb-title: Campaign technotes
---

# Adobe Campaign Technotes {#technotes-ac}

+ [Campaign technotes](technotes-home.md)
+ Upgrades{#tn-new}
  + {hide-from-toc} [Push Notification Channel changes](upgrades/push-technote.md)  
  + [Migrate users to IMS](upgrades/migrate-users-to-ims.md)
  + [Migrate technical operators to Developer Console](upgrades/ims-migration.md)
  + {hide-from-toc} [Migrate technical operator to Developer Console](upgrades/ims-migration-old.md)
  + [Move to the new SMS connector](upgrades/sms-migration.md)  
  + {hide-from-toc} [Campaign sending infrastructure migration to AWS](upgrades/migrate-to-aws.md)
  + {hide-from-toc} [Campaign Email Sending Infrastructure Upgrade](upgrades/upgrade-to-aws.md)
  + {hide-from-toc} [Upgrade Apache version](upgrades/apache.md)
  + {hide-from-toc} [Internet browsers](upgrades/browsers.md)
  + {hide-from-toc} [2023 system upgrades](upgrades/tech-stack-upgrade.md)
  + {hide-from-toc} [client console 32-bit deprecation](upgrades/console.md)
  + Security {#security}
    + {hide-from-toc} [Customer credentials rotation](security/credential-rotation-guide.md)
    + {hide-from-toc} [Asymetric encryption](security/asymetric-encryption.md)
+ [< Back to Campaign v8 documentation](https://experienceleague.adobe.com/en/docs/campaign/campaign-v8/campaign-home)
