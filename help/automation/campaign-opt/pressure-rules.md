...
* A threshold is the highest number of deliveries authorized for a given recipient within a given period. It can be either set or variable. It is set or calculated in the typology rule settings. [Learn more](#maximum-number-of-messages).
* Delivery weights let you identify top-priority deliveries within the framework of pressure management. Messages with the highest weight have priority. [Learn more](#message-weight).
...
Otherwise, the recipient will be **[!UICONTROL Excluded by arbitration]**. [Learn more](#exclusion-after-arbitration).

## Create a pressure rule {#create-a-pressure-rule}
...
1. Change the execution order if needed. When multiple typology rules are applied as a **[!UICONTROL Typology]** set, the lower ordered rules are applied first. [Learn more](apply-rules.md#execution-order).
1. In the **[!UICONTROL Calculation parameters]** section, define a frequency if you want to save targeting beyond the next daily re-arbitration execution. [Learn more](apply-rules.md#adjust-calculation-frequency).
...
1. Specify the method for calculating delivery weight.
   ...
   Each delivery has a weight, i.e. a value which represents its priority level: this enables arbitration between campaigns. Weights are calculated using the formula defined in the typology rule and/or in its properties. [Learn more](#message-weight).
...
1. The **[!UICONTROL Typologies]** tab lets you view the campaign typologies which apply this rule or link the rule to one or more existing typologies. [Learn more](campaign-typologies.md#apply-typologies).

## Define thresholds and weights {#define-thresholds-and-weights}
...
>[!NOTE]
>
>When you change the definition of a typology rule, you can create a **Simulation** to control its impact on the deliveries it is applied to and monitor the impact which the deliveries have on each other. [Learn more](campaign-simulations.md).
...
### Adapt the threshold based on criterion {#adapt-the-threshold-based-on-criterion}
...
### Calculate the delivery weight based on behavior {#calculate-the-delivery-weight-based-on-behavior}
...
### Send only the highest-weighted messages {#send-only-the-highest-weighted-messages}
...
1. Create a pressure rule. [Learn more](#create-a-pressure-rule).
...
1. Create a campaign. [Learn more](../campaigns/marketing-campaign-create.md#create-a-campaign).
1. In the **[!UICONTROL Targeting and workflows]** tab of your campaign, add a **Query** activity to your workflow. For more on using this activity, refer to [this section](../workflow/query.md).
1. Add an **[!UICONTROL Email delivery]** activity to the workflow and open it. For more on using this activity, refer to [this section](../workflow/delivery.md).
...
1. Set the contact date to the next day (current date + 1 day).
   ...
   For the pressure rule exclusions to be implemented successfully, make sure you set the extraction date and time before the contact date and time, as well as before the nightly arbitration is re-applied. [Learn more](#exclusion-after-arbitration).
...
When the nightly arbitration is applied, the deliveries with the lower weights for the same recipient will be excluded. Only the deliveries with the highest weight will be considered for send. [Learn more](#message-weight).
