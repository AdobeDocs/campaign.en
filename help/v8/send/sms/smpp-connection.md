...
First of all, remember that the SMS connector involves 3 entities : you, the SMPP provider and Adobe. The main SMS expert here is the SMPP provider, so they should be involved first for all SMS traffic-related issues (connection issues, lost messages, encoding problems, country-specific rules, ...).

#### Enable dedicated process

>[!IMPORTANT]
>
>Make sure dedicated process is enabled in all active SMS accounts. 
...
Ask the provider to check that they properly allowed IP addresses of the Campaign instance (most providers require this).
