...
* Resolved an issue where dynamic report data was missing after migration. (NEO-92962)
* Fixed an issue where delivery status was not updating correctly. (NEO-92908)
* Added a workaround for Databricks FDA USE CATALOG restriction. (NEO-92900)
* Fixed an issue where the Visual Editor broke the HTML layout in Outlook Windows desktop. (NEO-92611)
* Resolved a critical data integrity issue where delivery primary keys were duplicated on mid instance after upgrade. (NEO-92424)
* Fixed an issue where links could not be disabled in the Tracking & Images Dialog Box in a delivery. (NEO-92381)
* Corrected an issue where the nms.subscribtion.RecipientSubscribe() function did not work for bulk subscription. (NEO-92308)
* Resolved an issue where delivery failures occurred due to missing delivery parts after upgrade. (NEO-92278)
* Fixed an issue in the tracking workflow. (NEO-92239)
* Resolved an issue where temporary enum references were missing in the List XML after list creation using a workflow. (NEO-91158)
* Fixed an issue where the RT Publish/Unpublish dialog did not close and froze. (NEO-91038)
* Resolved an issue where recipients stuck with "Taken into account by the Service provider" status were not reaching Momentum. (NEO-90927)
* Fixed an issue where the (Un)Subscription Origin was missing in v8 for opt-out links. (NEO-90714)
* Corrected an issue where adding coupons failed delivery preparation. (NEO-90547)
* Fixed an issue where the Insert Reject Count was not accurately reflected in the Audit tab. (NEO-90318)
* Resolved a security issue that could cause application denial of service. (NEO-89984)
* Fixed an issue where the downloaded PDF of the Hotclick Report was broken. (NEO-89954)
* Resolved an SSL error that occurred after upgrade, causing unexpected EOF while reading errors. (NEO-89108)
* Fixed an issue where data could not be queried in data schema after upgrade. (NEO-88663)
* Corrected an error that occurred when concatenating a "char" field in PostgreSQL 15. (NEO-88028)
* Fixed an issue where the order of delivery template variables changed when saving or duplicating the template. (NEO-87845)
* Resolved an issue where creating a new Data Library schema caused the Web interface to crash. (NEO-87816)
* Fixed an issue where the Deduplication activity's complement set segment code was not working. (NEO-87711)
* Resolved a request for installation package without X11 dependency. (NEO-87471)
* Fixed an issue where segment codes could not be used in dynamic reports. (NEO-87276)
* Resolved an issue where workflows got stuck in the Update Data activity. (NEO-87252)
* Fixed an issue where BigQuery was using an incorrect timezone. (NEO-86622)
* Corrected a JavaScript error that occurred while evaluating the script 'mcSynch_mcExec1/jsReplicateUrl'. (NEO-86553)
* Resolved an issue where duplicate events appeared in the eventHisto table due to identifier calculation method. (NEO-86544)
* Fixed an issue where the Advanced tab was not showing for iOS Push upon copy. (NEO-86231)
* Resolved an issue where the Replicate Reference tables workflow failed in replicating the nms:delivery schema. (NEO-85884)
...
* Added a way to refresh technical delivery templates after any changes that may have been made to options. (NEO-84149)
* Fixed an error in the out-of-the-box Billing workflow. (NEO-83624)
* Resolved an issue with exclusion of duplicates based only on the primary key of targeted records. (NEO-82910)
...
* Corrected an issue where the lastmodified date changed even if the record should not be updated in the Update Data activity. (NEO-82002)
* Fixed an issue where adding new attributes in a List caused workflows reading the list to fail. (NEO-80258)
* Resolved an anomaly in the Tracking Indicators Report. (NEO-79466)