...
* If you use custom code, security tokens are not retrieved from the GET URL securitytoken parameter anymore. You have to generate a new security token using the following JSSP code: `getNewSecurityToken(jsspContext.getSessionToken(), jsspContext.getSecurityToken(), true);` 
