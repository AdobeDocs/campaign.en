...
The primary key of Adobe Campaign tables is a **Universally Unique ID (UUID)** auto-generated by the database engine. The calculation of the key value depends on a sequence (by default, the **XtkNewId** SQL function) generating a number that is unique in the entire database. The content of the key is automatically entered on insertion of the record.

You can specify in the source schema the name of the sequence to be used with the **pkSequence** attribute. Campaign uses dedicated sequences for the **nms:broadLog** and **nms:trackingLog** schemas (**NmsBroadLogId** and **NmsTrackingLogId** respectively) because these are the tables that contain the most records.
