...
>[!NOTE]
>
>The **autouuid** attribute only applies to [Enterprise (FFDA) deployments](../architecture/enterprise-deployment.md).
...
In the context of an [Enterprise (FFDA) deployment](../architecture/enterprise-deployment.md), Adobe Campaign primary key is an auto-generated UUID for all built-in tables. A UUID can also be used for custom tables. [Learn more](../architecture/keys.md)
...
Declaring a link as an external join is not good for performance. The zero-id record emulates the external join functionality. In the context of an [Enterprise (FFDA) deployment](../architecture/enterprise-deployment.md), it is not necessary to declare external joins if the link uses the **autouuid**.
...
![](../assets/do-not-localize/speech.png)  As a Managed Cloud Services user, reach out to the Adobe consultants or technical administrators to learn more about retention or if you need to set retention for custom tables.
