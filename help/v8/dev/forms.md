...
>[!NOTE]
>
>You can overwrite the label defined in its data schema by adding the **label** attribute to the `<input>` element:  
...
![](../assets/do-not-localize/book.png) All form attributes are listed in [Campaign Classic v7 documentation](https://docs.adobe.com/content/help/en/campaign-classic/technicalresources/api/control-Button.html).
...
Use **containers** to group a set of controls. They are represented by the **`<container>`** element. They were used above to format controls over several columns.
...
### Add tabs (notebook) {#tab-container}

Use a **notebook** container to format data in pages that are accessible from tabs.
...
Add the **style="down"** attribute to force the vertical positioning of tab labels below the control. This attribute is optional. Default value is **"up"**.

![](assets/do-not-localize/form_exemple7.png) 

`<container style="down" type="notebook">  ... </container>`

### Add icons (iconbox) {#icon-list}

Use this container to display a vertical icon bar that lets you select the pages to be displayed.
...
### Hide containers (visibleGroup) {#visibility-container}

You can hide a set of controls via a dynamic condition.
...
### Conditional display (enabledGroup) {#enabling-container}
...
## Add a list of links (unbound) {#list-of-links}
...
## Define a relationship table {#relationship-table}
...
## Add a memory list controls {#memory-list-controls}
...
## Add non-editable fields {#non-editable-fields}
...
## Edit navigation hierarchy {#navigation-hierarchy-edit}
...
## Add an expression field {#expression-field}
...
### Update the context {#updating-the-context}
...
To add an assistant, use the following type of structure:
