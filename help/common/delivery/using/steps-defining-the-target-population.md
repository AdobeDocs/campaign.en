---
product: campaign
title: Define the audience of a delivery in Campaign
description: Define the delivery target population
audience: delivery
...
## Select the main audience of a delivery {#selecting-the-main-target}
...
1. Click the **[!UICONTROL Add]** button to define restriction filters.
    ...
    * **[!UICONTROL Filtering conditions]** : this option lets you define a query and display the result. The method for defining queries is presented in [this section](../../../v7/platform/using/creating-filters.md#creating-an-advanced-filter).
    ...
    * **[!UICONTROL User filters]** : this option lets you access the preconfigured filters to use them as filtering criteria for profiles in the database. Preconfigured filters are presented in [this section](../../../v7/platform/using/creating-filters.md#saving-a-filter).
...
### Send messages to external recipients {#selecting-external-recipients}

You can send a delivery on recipients who are not saved in the database, but stored in an external file. 

In the example below, you will send here a delivery to recipients imported from a text file. Follow the steps below:
...
### Customize exclusion settings {#customizing-exclusion-settings}

Address errors and quality ratings are supplied by the service provider (ISP). This information is automatically updated in the recipient profile after delivery execution.

You can choose to exclude addresses that have reached a certain number of consecutive errors, or whose quality rating is below a threshold specified in the  **[!UICONTROL Exclusions]**  tab of the **[!UICONTROL Select target]** window. You can also define whether or not to send messages to non-qualified addresses, i.e for which no data has been returned.

The **[!UICONTROL Exclusions]** tab is also used to limit the number of messages.

>[!CAUTION]
>
>Default parameters are recommended, but you can adapt settings depending on your implementation and requirements. However, these options should only be changed by an expert user to avoid any misusage or errors.
...
### Limit the size of the audience {#reduce-the-size-of-the-audience}

For a direct mail delivery, you can reduce the size of the target population. To do this, specify the number of recipients to be exported in the **[!UICONTROL Requested quantity]** field.
...
Select this option to choose recipients using the **[!UICONTROL Add]** button, as in the case of defining the main target. [Learn more](steps-defining-the-target-population.md#selecting-the-main-target).

![](assets/s_ncs_user_wizard_email01_143.png)

Learn more about proofs, in [this section](steps-validating-the-delivery.md#sending-a-proof).
