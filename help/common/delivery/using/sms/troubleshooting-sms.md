...
* If the connection is successful but unstable, check the [Issue with unstable connections](#issues-unstable-connection) section.
...
* If restarting the MTA makes sending MT working again for a small period of time, you probably have throttling due to an unstable connection. [Learn more](#issues-unstable-connection).
...
* If you see many `BIND/UNBIND`, you have an unstable connection. [Learn more](troubleshooting-sms.md#issues-unstable-connection).
...
If the `DELIVER_SM PDU` has been acknowledged by the Adobe Campaign Classic extended SMPP connector but the broadLog is not updated properly, check the ID reconciliation process described in the [Matching MT, SR and broadlog entries](sms-protocol.md#matching-mt) section.
