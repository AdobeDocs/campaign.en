...
* The v2 connector which allows simultaneous connections to the FCM server to improve throughput (recommended)
...
1. In the **[!UICONTROL Connector]** tab, fill in the **[!UICONTROL JavaScript used in the connector]** field for Android v2: https://localhost:8080/nms/jsp/androidPushConnectorV2.js (or https://localhost:8080/nms/jsp/androidPushConnector.js for v1 connector)

   ![](../assets/nmac_connectors3.png)


>[!NOTE]
>For Android v2, an additional parameter is available in the Adobe Server configuration file (serverConf.xml): **maxGCMConnectPerChild**, which sets the maximum limit of parallel HTTP requests to the FCM initiated by each child server (8 by default).

## Configure an Android service {#configuring-android-service}

To configure the Android service in Campaign, follow the steps below:
...
1. Make sure the same **[!UICONTROL Integration key]** is defined in Adobe Campaign and in the application code via the SDK. For more on this, refer to [this section](integrating-campaign-sdk-into-the-mobile-application.md). The **[!UICONTROL Integration key]** is fully customizable with string value but needs to be exactly the same as the one specified in the SDK.
...
1. Create an extension of the **[!UICONTROL Subscriber applications (nms:appsubscriptionRcp)]** schema and define the new fields. Learn more about schema extension in [this page for Campaign Classic v7](../../../../v7/configuration/using/about-schema-edition.md) and in [this page for Campaign v8](../../../../v8/dev/schemas.md).
